...
from rbteacher.data.common import (
...
def divide_label_unlabel(
    ...
    #write down the random choice for object365 1% 2% 5% 10%
    # json_w = {}
    # for percent in [1]:
    #     num_label = int(percent/100.0*num_all)
    #     labeled_idx = np.random.choice(range(num_all),size=num_label,replace=False)
    #     json_w [str(percent)] = labeled_idx.tolist()

    # with open(random_data_seed_path,'w') as f:
    #     json.dump(json_w,f)

    # print(SupPercent,random_data_seed)
    # print('+++++++++++++++++++++++++++++++++',num_all,labeled_idx.shape)

    # labeled_idx = np.array(coco_random_idx[str(int(SupPercent))])
